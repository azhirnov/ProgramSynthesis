...
*/
	...
	bool BruteforceGenerator::_Gen (ArrayCRef<TestCase<T>> testCases)
	{
		...
		GpuMsg::GetDeviceProperties		req_props;
...
#extension GL_GOOGLE_include_directive : require
		...
		// fitness function
		{
			_temp_src[1] = FitnessFunctions::GetSource( _config.fitnessFunc );
...
*/
	bool BruteforceGenerator::_CreateResources ()
	{
		...
		_syncManager	= _gpuThread->GetModuleByMsg<CompileTime::TypeListFrom< GpuMsg::CreateFence >>();
		...
		CHECK_ERR( factory->Create(
						...
						CreateInfo::GpuBuffer{
							BufferDescription{ _inputBufferSize, EBufferUsage::Storage | EBufferUsage::TransferDst },
		...
		CHECK_ERR( factory->Create(
						...
						_gpuThread->GlobalSystems(),
						CreateInfo::GpuBuffer{
							BufferDescription{ _inputBufferSize, EBufferUsage::Storage | EBufferUsage::TransferSrc },
		...
		CHECK_ERR( factory->Create(
						...
						_gpuThread->GlobalSystems(),
						CreateInfo::GpuBuffer{
							BufferDescription{ _outputBufferSize, EBufferUsage::Storage | EBufferUsage::TransferDst | EBufferUsage::TransferSrc },
							EGpuMemory::LocalInGPU,
							EMemoryAccess::GpuReadWrite },
						OUT _outBuffer ) );
		...
		CHECK_ERR( factory->Create(
						...
						_gpuThread->GlobalSystems(),
						CreateInfo::GpuBuffer{
							BufferDescription{ _outputBufferSize, EBufferUsage::Storage | EBufferUsage::TransferDst },
							EGpuMemory::CoherentWithCPU },
						OUT _outStagingBuffer ) );


		GpuMsg::CreateComputePipeline	cppl_ctor{ gpu_ids.pipeline, _gpuThread };
		...
		for (auto& cb : _cmdBuffers)
		{
			...
			CHECK_ERR( factory->Create(
						...
						_gpuThread->GlobalSystems(),
						CreateInfo::GpuBuffer{
							BufferDescription{ _atomicsSize, EBufferUsage::TransferDst },
							EGpuMemory::CoherentWithCPU },
						OUT out_buf ) );
			...
			CHECK_ERR( factory->Create(
							...
							_gpuThread->GlobalSystems(),
							CreateInfo::GpuCommandBuffer{ CommandBufferDescription{ ECmdBufferCreate::ImplicitResetable } },
							OUT cmd_buf ) );
...
*/
	...
	bool BruteforceGenerator::_ProcessBlock (ArrayCRef<T> constants)
	{
		...
		if ( first_call )
		{
			_cmdBuilder->Send( GpuMsg::CmdCopyBuffer{ _inStagingBuffer, _inBuffer }.AddRegion( 0_b, 0_b, _inputBufferSize ));
		...
		_cmdBuilder->Send( GpuMsg::CmdCopyBuffer{ _outBuffer, curr_buf.outputBuf }.AddRegion( 0_b, 0_b, _atomicsSize ));

		_cmdBuilder->Send( GpuMsg::CmdEnd{} );


		GpuMsg::SubmitComputeQueueCommands	submit;
...
*/
	...
	bool BruteforceGenerator::_ProcessBlockWithConst ()
	{
		...
		// update progress
		{
			ConstantHash_t	chash				= _constHash;
			const uint		c_count				= uint(const_args.Count());

			chash.Write( 0, _maxConstBits - const_count_bits, const_count_bits );
			
			for (usize i = c_count; i < _config.maxConstants; ++i)
			{
				chash.Write( 0, bits_per_const * i, bits_per_const );
			}

			const BitsU		const_bits			= chash.Count(); //Max( chash.Count(), (c_count ? IntLog2( c_count ) : 0u) << (_maxConstBits - const_count_bits) );

			const double	total_fun_ticks		= Pow( 2.0, double(usize(_maxBits)) );
			const double	total_const_ticks	= Pow( 2.0, double(usize(_maxConstBits - const_count_bits)) );
			const double	total_ticks			= total_fun_ticks * total_const_ticks;

			const double	curr_fun_ticks		= Pow( 2.0, double(usize(_funHash.Count())) );
			const double	curr_const_ticks	= Pow( 2.0, double(usize(const_bits)) );
			const double	curr_ticks			= curr_fun_ticks * curr_const_ticks;

			const double	progress			= curr_ticks / total_ticks * 100.0;
...
*/
	bool BruteforceGenerator::_CopyOutputToStagingBuffer ()
	{
		...
		_cmdBuilder->Send( GpuMsg::CmdBegin{ curr_buf.cmd });
		_cmdBuilder->Send( GpuMsg::CmdCopyBuffer{ _outBuffer, _outStagingBuffer }.AddRegion( 0_b, 0_b, _outputBufferSize ));
		_cmdBuilder->Send( GpuMsg::CmdEnd{} );
